public class AVLTree<E extends Comparable<E>> extends BST<E> {
	...
	*/
	private void balancePath(E e) {
		java.util.ArrayList<TreeNode<E>> path_t = path(e);

		//System.out.println(path_t);

		for (int i = path_t.size() - 1; i >= 0; i--) {
			AVLTreeNode<E> A = (AVLTreeNode<E>)(path_t.get(i));
			updateHeight(A);
			AVLTreeNode<E> parentOfA = (A == root) ? null :	(AVLTreeNode<E>)(path_t.get(i - 1));
			//System.out.println(balanceFactor(A));
			//System.out.println(parentOfA);
			switch (balanceFactor(A)) {
			case -2:
				if (balanceFactor((AVLTreeNode<E>)A.left) <= 0) {
					balanceLL(A, parentOfA); // Perform LL rotation
					//System.out.println("balanceLL");
				}
				else {
